...
const SellerLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ✅ SELLER LOGIC: Send login request to the SELLER endpoint
      const response = await axiosInstance.post(
        "/seller/login", // Changed from /buyer/login
        data
      );

      // If token is received, log in and redirect
      if (response.data.token) {
        // ✅ SELLER LOGIC: Log in with seller-specific data
        login(
          {
            name: response.data.name,
            email: data.email, // API doesn't return email, so we use the form's email
            role: response.data.role,
            businessName: response.data.businessName, // Add seller-specific field
          },
          response.data.token
        );

        // ✅ SELLER LOGIC: Redirect to the seller account dashboard
        navigate("/seller-account"); // Changed from "/"
      } else {
  ...
  // JSX for login form UI (Copied from Buyer Login)
  return (
          ...
          <div className="mb-6">
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <p className="mt-4 text-center text-sm text-[#7F7F7F]">
                ...
                <Link
                  // ✅ SELLER LOGIC: Link to the seller registration page
                  to="/become-seller" // Changed from /signup
